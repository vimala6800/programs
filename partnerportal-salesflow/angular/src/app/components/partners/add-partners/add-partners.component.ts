...
export class AddPartnersComponent implements OnInit {
  ...
  adduserId: any;
  constructor(private partnerService: PartnersService,private toastr:ToastrService, private route: ActivatedRoute, private router: Router) {
  ...
  tick() {
    if (this.addressCheckBox == true) {
      ...
      this.addpartnerRequest.billingAddress1 = '';
    }
    else {
      this.addressCheckBox = true;
      this.addpartnerRequest.billingAddress1 = this.addpartnerRequest.address1;
    }
  }
 
  async addPartner() {
    let users = await this.partnerService.addUser(this.addUserRequest);
    
    this.addpartnerRequest.spocUserID = users?.message?.slice(13, 49);
    await this.partnerService.addPartner(this.addpartnerRequest);
    this.success();
