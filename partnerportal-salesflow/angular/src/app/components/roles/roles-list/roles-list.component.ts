...
export class RolesListComponent {
  ...
  chkRoleExists(): boolean {
    let userRole = this.roles;
    let isRoleExists = false;
    for (let i = 0; i < userRole.length; i++) {
      if (userRole[i].name == this.addR.name) {
        isRoleExists = true;
      }
    }
    return isRoleExists;
  ...
  showMessage(msg) {
    this.dataService.showError(msg, "");
  }
  addRole(): void {
    this.nameValidator();
    this.descValidator();
    ...
    if (this.validator.name == "" && this.validator.desc == "") {
      let isRoleExists = this.chkRoleExists();
      if (isRoleExists == true) {
        this.showMessage("Role Exists")
      }
      else {
      ...
      next: (user) => {
        ...
        for (let index = 0; index < this.functionalities.length; index++) {
          this.dataService.addPermission({
            ...
            fId: String(this.functionalities[index].fId),
            pId: String(this.functionalities[index].fId) 
          }).subscribe({
            next: (permission) => {
              console.log("success");
            }
