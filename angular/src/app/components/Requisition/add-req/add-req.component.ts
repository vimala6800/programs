...
export class AddReqComponent implements OnInit {
  ...
  constructor(
    private listsClient: RequisitionClient, private projectmanagerservice: ProjectmanagerService, private route: ActivatedRoute, private requisitionService: RequisitionService, private router: Router, private partnerService: PartnersService, private http: HttpClient, private toastr: ToastrService) { }
  ngOnInit(): void {
  ...
  addRequisition(addRequisitionRequest: AddRequisition) {
    ...
    this.http.post<AddRequisition>('https://localhost:7184/api/Requisition', addRequisitionRequest)
      ...
      .subscribe(response => {
        console.log(response);
        this.toastr.success("Requisition Added Successfully");
